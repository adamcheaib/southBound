...
const uploadCarButton = document.querySelector(
  "#adminMainMenu > div:nth-child(1)"
);
const alreadyUploadedButton = document.querySelector(
  "#adminMainMenu > div:nth-child(2)"
);
...
async function fetchUploadPage(event) {
  ...
  try {
    ...
    function addMoreImageInputs() {
      ...
      fileInputContainer
        .querySelector(".removeImageButton")
        .addEventListener("click", function removeMoreImageInputs(event) {
...
async function uploadCar(event) {
  ...
  try {
    ...
    const bodyPost = new Request("./uploadFunction.php", {
      method: "POST",
      body: formData,
    });
    dialog.innerHTML = `
    <div id="uploadNotification">
      <h2 class="uploadingProcess">Uploading car</h2>
    </div>`;
    dialog.showModal();
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (resource.message.length == 0) {
        mainWrapper.innerHTML =
          "<h1 style='text-align: center'>No cars uploaded</h1>";
      }
